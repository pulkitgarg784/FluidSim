...
### Fresnel Effects
For the environment reflection and refractions, we will use an environment map and sample it to get reflection and refraction, and then use Fresnel laws to blend them dynamically.

### Water Depth Visuals
To render light absorption, we can first create a thickness FBO by adding particle depth along the view rate. Then using the Beer-Lambert Law ($I = I_0 \cdot e^{-\mu \cdot d}$), we can calculate the light absorption and color based on fluid depth, where $I$ is the intensity after passing through the liquid, $\mu$ is the absorption coefficient and $d$ is the thickness/depth. We can also project the thickness map from the light's view to get realistic shadows.
